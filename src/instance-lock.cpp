...
// A terminating editor drains clean-window capture so Niri's temporary
// clipboard ownership is restored before the lock is released.
constexpr int kHandoverTimeoutMs = 60000;
...
SignalOutcome terminateHolder(qint64 pid, QString &error) {
  ...
  error = QStringLiteral("Could not stop the running omarecord instance "
                         "(pid %1): %2")
              .arg(pid)
...
InstanceLockResult acquireInstanceLock(QLockFile &lock, InstanceMode mode) {
  ...
  case InstanceAction::ReplaceRunning:
    ...
    return {false, kInstanceLockErrorExitCode,
            QStringLiteral("Timed out waiting for omarecord (pid %1) to release "
